...
class Models():
    ...
    def detect_scrdf(self, img, max_num, score):
        ...
        io_binding.bind_input(name=input_name, device_type='cpu', device_id=0, element_type=np.float32,  shape=det_img.size(), buffer_ptr=det_img.data_ptr())
        
        for i in range(len(output_names)):
            io_binding.bind_output(output_names[i], 'cpu')
    ...
    def detect_yoloface(self, img, max_num, score):
        ...
        image = torch.zeros((length, length, 3), dtype=torch.uint8, device='cpu')
        img = img.permute(1,2,0)
        ...
        io_binding.bind_input(name='images', device_type='cpu', device_id=0, element_type=np.float32,  shape=image.size(), buffer_ptr=image.data_ptr())
    ...
    def detect_yoloface2(self, image_in, max_num, score):
        ...
        io_binding = self.yoloface_model.io_binding()
        io_binding.bind_input(name='images', device_type='cpu', device_id=0,
    ...
    def recognize(self, img, face_kps):
        ...
        io_binding.bind_input(name=input_name, device_type='cpu', device_id=0, element_type=np.float32,  shape=img.size(), buffer_ptr=img.data_ptr())
    ...
    def resnet50(self, image, score=.5):   
        ...
        mean = torch.tensor([104, 117, 123], dtype=torch.float32, device='cpu')
        ...
        conf = torch.empty((1,10752,2), dtype=torch.float32, device='cpu').contiguous()
        landmarks = torch.empty((1,10752,10), dtype=torch.float32, device='cpu').contiguous()

        io_binding = self.resnet50_model.io_binding() 
        io_binding.bind_input(name='input', device_type='cpu', device_id=0, element_type=np.float32, shape=(1,3,512,512), buffer_ptr=image.data_ptr())
        io_binding.bind_output(name='conf', device_type='cpu', device_id=0, element_type=np.float32, shape=(1,10752,2), buffer_ptr=conf.data_ptr())
        io_binding.bind_output(name='landmarks', device_type='cpu', device_id=0, element_type=np.float32, shape=(1,10752,10), buffer_ptr=landmarks.data_ptr())
