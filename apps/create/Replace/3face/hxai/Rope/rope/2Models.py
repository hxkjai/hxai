...
class Models():
    def __init__(self): 
        ...
        self.gfpgan_1_2_model = []
        self.gfpgan_1_3_model = []  # 添加 gfpgan_1.3 模型
        self.gpen_bfr_1024_model = []  # 添加 gpen_bfr_1024 模型
        self.gpen_bfr_2048_model = []  # 添加 gpen_bfr_2048 模型
        self.restoreformer_plus_plus_model = []  # 添加 restoreformer_plus_plus 模型
        self.simswap_512_model = []  # 初始化 SimSwap 模型属性
    ...
    def run_detect(self, img, detect_mode='Retinaface', max_num=1, score=0.5):
        ...
        if detect_mode=='Retinaface':
            if not self.retinaface_model:
                self.retinaface_model = onnxruntime.InferenceSession('../models/models1/det_10g.onnx', providers=self.providers)
            ...
            if not self.yoloface_model:
                self.yoloface_model = onnxruntime.InferenceSession('../models/models1/yoloface_8n.onnx', providers=self.providers)
                self.insight106_model = onnxruntime.InferenceSession('../models/models1/2d106det.onnx', providers=self.providers)
    ...
    def delete_models(self):
        ...
        self.GPEN_512_model = []
        self.codeformer_model = []
        self.gfpgan_1_2_model = []  # 添加 gfpgan_1.2 模型
        self.gfpgan_1_3_model = []  # 添加 gfpgan_1.3 模型
        self.gpen_bfr_1024_model = []  # 添加 gpen_bfr_1024 模型
        self.gpen_bfr_2048_model = []  # 添加 gpen_bfr_2048 模型
        self.restoreformer_plus_plus_model = []  # 添加 restoreformer_plus_plus 模型
    ...
    def run_recognize(self, img, kps):
        if not self.recognition_model:
            self.recognition_model = onnxruntime.InferenceSession('../models/models1/w600k_r50.onnx', providers=self.providers)
    ...
    def calc_swapper_latent(self, source_embedding):
        if not self.swapper_model:
            graph = onnx.load("../models/models1/inswapper_128.fp16.onnx").graph
    ...
    def run_simswap_512(self, image, embedding, output):
        if not self.simswap_512_model:
            self.simswap_512_model = onnxruntime.InferenceSession(
                "../models/models/simswap_512_unofficial.onnx", providers=self.providers)

        io_binding = self.simswap_512_model.io_binding()
        io_binding.bind_input(name='target', device_type='cuda', device_id=0,
                              element_type=np.float32, shape=(1, 3, 512, 512),
                              buffer_ptr=image.data_ptr())
        io_binding.bind_input(name='source', device_type='cuda', device_id=0,
        ...
        self.syncvec.cpu()
        self.simswap_512_model.run_with_iobinding(io_binding)
    ...
    def run_swapper(self, image, embedding, output):
        if not self.swapper_model:
            ...
            # self.swapper_model = onnxruntime.InferenceSession( "../models/models1/inswapper_128_last_cubic.onnx", sess_options, providers=[('CUDAExecutionProvider', cuda_options), 'CPUExecutionProvider'])

            self.swapper_model = onnxruntime.InferenceSession( "../models/models1/inswapper_128.fp16.onnx", providers=self.providers)
    ...
    def run_swap_stg1(self, embedding):
        ...
        if not self.swapper_model_kps:
            self.swapper_model_kps = onnxruntime.InferenceSession( "../models/models1/inswapper_kps.onnx", providers=self.providers)
    ...
    def run_swap_stg2(self, image, holder, output):
        if not self.swapper_model_swap:
            self.swapper_model_swap = onnxruntime.InferenceSession( "../models/models1/inswapper_swap.onnx", providers=self.providers)
    ...
    def run_GFPGAN(self, image, output):
        if not self.GFPGAN_model:
            ...
            # self.GFPGAN_model = onnxruntime.InferenceSession( "../models/models1/GFPGANv1.4.onnx", sess_options, providers=[("CUDAExecutionProvider", cuda_options), 'CPUExecutionProvider'])

            self.GFPGAN_model = onnxruntime.InferenceSession( "../models/models1/GFPGANv1.4.onnx", providers=self.providers)
    ...
    def run_GPEN_512(self, image, output):
        if not self.GPEN_512_model:
            self.GPEN_512_model = onnxruntime.InferenceSession( "../models/models1/GPEN-BFR-512.onnx", providers=self.providers)
    ...
    def run_GPEN_256(self, image, output):
        if not self.GPEN_256_model:
            self.GPEN_256_model = onnxruntime.InferenceSession( "../models/models1/GPEN-BFR-256.onnx", providers=self.providers)
    ...
    def run_codeformer(self, image, output):
        if not self.codeformer_model:
            self.codeformer_model = onnxruntime.InferenceSession( "../models/models1/codeformer_fp16.onnx", providers=self.providers)
    ...
    def run_gfpgan_1_2(self, image, output):
        if not self.gfpgan_1_2_model:
            self.gfpgan_1_2_model = onnxruntime.InferenceSession(
                "../models/models/gfpgan_1.2.onnx", providers=self.providers)

        io_binding = self.gfpgan_1_2_model.io_binding()
        io_binding.bind_input(name='input', device_type='cuda', device_id=0,
                              element_type=np.float32, shape=(1, 3, 512, 512),
                              buffer_ptr=image.data_ptr())
        io_binding.bind_output(name='output', device_type='cuda', device_id=0,
                              element_type=np.float32, shape=(1, 3, 512, 512),
        ...
        self.syncvec.cpu()
        self.gfpgan_1_2_model.run_with_iobinding(io_binding)

    def run_gfpgan_1_3(self, image, output):
        if not self.gfpgan_1_3_model:
            self.gfpgan_1_3_model = onnxruntime.InferenceSession(
                "../models/models/gfpgan_1.3.onnx", providers=self.providers)

        io_binding = self.gfpgan_1_3_model.io_binding()
        io_binding.bind_input(name='input', device_type='cuda', device_id=0,
                              element_type=np.float32, shape=(1, 3, 512, 512),
                              buffer_ptr=image.data_ptr())
        io_binding.bind_output(name='output', device_type='cuda', device_id=0,
        ...
        self.syncvec.cpu()
        self.gfpgan_1_3_model.run_with_iobinding(io_binding)

    def run_gpen_bfr_1024(self, image, output):
        if not self.gpen_bfr_1024_model:
            self.gpen_bfr_1024_model = onnxruntime.InferenceSession(
                "../models/models/gpen_bfr_1024.onnx", providers=self.providers)

        io_binding = self.gpen_bfr_1024_model.io_binding()
        io_binding.bind_input(name='input', device_type='cuda', device_id=0,
                              element_type=np.float32, shape=(1, 3, 1024, 1024),
                              buffer_ptr=image.data_ptr())
        ...
        self.syncvec.cpu()
        self.gpen_bfr_1024_model.run_with_iobinding(io_binding)

    def run_gpen_bfr_2048(self, image, output):
        if not self.gpen_bfr_2048_model:
            self.gpen_bfr_2048_model = onnxruntime.InferenceSession(
                "../models/models/gpen_bfr_2048.onnx", providers=self.providers)

        io_binding = self.gpen_bfr_2048_model.io_binding()
        io_binding.bind_input(name='input', device_type='cuda', device_id=0,
                              element_type=np.float32, shape=(1, 3, 2048, 2048),
                              buffer_ptr=image.data_ptr())
        io_binding.bind_output(name='output', device_type='cuda', device_id=0,
                               element_type=np.float32, shape=(1, 3, 2048, 2048),
                               buffer_ptr=output.data_ptr())
        ...
        self.syncvec.cpu()
        self.gpen_bfr_2048_model.run_with_iobinding(io_binding)



    def run_occluder(self, image, output):
        if not self.occluder_model:
            self.occluder_model = onnxruntime.InferenceSession("../models/models1/occluder.onnx", providers=self.providers)
    ...
    def run_faceparser(self, image, output):
        if not self.faceparser_model:
            self.faceparser_model = onnxruntime.InferenceSession("../models/models1/faceparser_fp16.onnx", providers=self.providers)
    ...
    def detect_yolo00face2(self, image_in, max_num, score):
        ...
        if result_boxes is not None and len(result_boxes) > 0:
            ...
            blob = cv2.dnn.blobFromImage(aimg, 1.0 / input_std, input_size, (input_mean, input_mean, input_mean),
                                         swapRB=True)
    ...
    def resnet50(self, image, score=.5):
        if not self.resnet50_model:
            self.resnet50_model = onnxruntime.InferenceSession("../models/models1/res50.onnx", providers=self.providers)
