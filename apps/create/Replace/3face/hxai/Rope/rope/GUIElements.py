...
from tkinter import ttk  # 添加导入 ttk 模块
...
class Separator_x():
    def __init__(self, parent, x, y):
        ...
        self.sep = tk.Label(self.parent, bg='#090909', image=self.blank, compound='c', border=0,
                            width=self.parent.winfo_width(), height=1)
...
class Separator_y():
    def __init__(self, parent, x, y):
        ...
        self.parent.update()
        self.blank = tk.PhotoImage()
        self.sep = tk.Label(self.parent, bg='#090909', image=self.blank, compound='c', border=0, width=1,
...
class Text():
    def __init__(self, parent, text, style_level, x, y, width, height):
        ...
        self.label = tk.Label(parent, self.style, image=self.blank, compound='c', text=text, anchor='w', width=width,
                              height=height)
...
class Button():
    def __init__(self, parent, name, style_level, function, args, data_type, x, y, width=125, height=20, command=None):
        ...
        # 这里修改：将 command 绑定到 button 创建之后
...
class TextSelection():
    def __init__(self, parent, name, display_text, style_level, function, argument, data_type, width, height, x, y,
                 text_percent):
        self.blank = tk.PhotoImage()
        ...
        # 创建 Label
        self.text_label = tk.Label(self.ts_frame, self.text_style, image=self.blank, compound='c',
                                   text=self.display_text, anchor='e', width=self.text_width, height=height)
        self.text_label.place(x=0, y=0)

        # 创建下拉菜单
        self.selection = tk.StringVar(self.ts_frame)  # 创建 StringVar 对象
        self.selection.set(self.default_data[self.name + 'Mode'])  # 设置默认值

        self.combobox = ttk.Combobox(self.ts_frame, textvariable=self.selection,
                                    values=self.default_data[self.name + 'Modes'])
        self.combobox.place(x=self.text_width + 20, y=0, width=self.width - self.text_width - 50, height=15)
        self.combobox.bind("<<ComboboxSelected>>", lambda event: self.select_ui_text_selection(
            self.selection.get(), request_frame=True))

        # 设置下拉菜单的样式
        self.style = ttk.Style()  # 将 style 对象设置为类属性
        self.style.theme_use('default')  # 强制使用默认主题
        # self.style.configure("TCombobox", background="#28282E", foreground="black",  # 设置深灰色背景
        #                      fieldbackground="#28282E", selectbackground="#28282E",
        #                    )  # 设置默认黑色字体颜色
        ...
        # 绑定点击事件
        self.combobox.bind("<Button-1>", lambda event: self.combobox.event_generate("<<ComboboxSelected>>"))
    ...
    def unhide(self):
        ...
        self.text_label.place(x=self.text_label.winfo_x(), y=self.text_label.winfo_y())
        self.combobox.place(x=self.combobox.winfo_x(), y=self.combobox.winfo_y())
    ...
    def select_ui_text_selection(self, selection, request_frame=True):
        ...
        for mode in self.default_data[self.name + 'Modes']:
            ...
            if mode == selection:
                # Set state to true
                self.selection.set(mode)  # 更新 StringVar 的值
                if request_frame:
...
class Switch2():
    def __init__(self, parent, name, display_text, style_level, function, argument, width, height, x, y):
        ...
        # Toggle Switch
        self.switch = tk.Label(self.switch_frame, style.parameter_switch_3, image=self.icon_off, width=toggle_width,
                               height=self.height)
        if self.state:
        ...
        # Text
        self.switch_text = tk.Label(self.switch_frame, style.parameter_switch_3, image=self.blank, compound='right',
                                    text=self.display_text, anchor='w', width=text_width, height=height - 2)
        self.switch_text.place(x=50, y=0)

    def toggle_switch(self, event, set_value=None, request_frame=True):
        # flip state
       # print(f"开关状态变化：{set_value}，请求帧：{request_frame}")
        if set_value == None:
            self.state = not self.state
        else:
...
class Slider2():
    def __init__(self, parent, name, display_text, style_level, function, argument, width, height, x, y,
        ...
        self.label = tk.Label(self.frame, self.text_style, image=self.blank, compound='c', text=self.display_text,
                              anchor='e', width=self.txt_label_width, height=self.height)
        ...
        self.slider.create_rectangle(self.trough_x1, self.trough_y1, self.trough_x2, self.trough_y2, fill='#1F1F1F',
                                     outline='')
        self.handle = self.slider.create_oval(self.oval_x1, self.oval_y1, self.oval_x2, self.oval_y2, fill='#919191',
                                              outline='')

    def coord2pos(self, coord):
        return float(
            (coord - self.slider_left) * (self.max_ - self.min_) / (self.slider_right - self.slider_left) + self.min_)

    def pos2coord(self, pos):
        return float((float(pos) - self.min_) * (self.slider_right - self.slider_left) / (
                    self.max_ - self.min_) + self.slider_left)
...
class Text_Entry():
    ...
    def send_text(self, text):
        if self.entry.focus_get():  # 仅在输入框获得焦点时执行
            self.function(self.data_type, self.name, use_markers=True)
        self.entry.master.master.master.master.master.focus_set()  # 将焦点设置回主窗口
...
class VRAM_Indicator():
    def __init__(self, parent, style_level, width, height, x, y):
        ...
        self.label_name = tk.Label(self.frame, self.frame_style, image=self.blank, compound='c', fg='#b1b1b2',
        ...
        self.canvas = tk.Canvas(self.frame, self.frame_style, highlightthickness=2, highlightbackground='#b1b1b2',
