...
def get_webcam_capture(webcam_index: int) -> Optional[cv2.VideoCapture]:
	global WEBCAM_CAPTURE

	if WEBCAM_CAPTURE is None:
		if platform.system().lower() == 'windows':
			webcam_capture = cv2.VideoCapture(webcam_index, cv2.CAP_DSHOW)
		else:
...
def listen() -> None:
	...
	webcam_index = get_ui_component('webcam_selector')
	if webcam_mode_radio and webcam_resolution_dropdown and webcam_fps_slider:
		start_event = WEBCAM_START_BUTTON.click(start, inputs = [ webcam_mode_radio, webcam_resolution_dropdown, webcam_fps_slider, webcam_index], outputs = WEBCAM_IMAGE)
...
def start(webcam_mode : WebcamMode, webcam_resolution : str, webcam_fps : Fps, webcam_index: int) -> Generator[VisionFrame, None, None]:
	...
	webcam_capture = get_webcam_capture(webcam_index)
	if webcam_capture and webcam_capture.isOpened():
